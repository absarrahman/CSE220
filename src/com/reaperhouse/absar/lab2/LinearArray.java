package com.reaperhouse.absar.lab2;


public class LinearArray{
    public static void main(String [] args){
        int [] a = {15,37,33,47,28,37,31,42};
        ...
        printArray(c); // This Should Print: { 10, 20, 70, 30, 40, 50, 0 }
        bol=insert(c,6, 70, 5); // This Should Print: Number of elements after insertion: 7
        System.out.println(bol); // This Should Print: true
    ...
    public static void reverseArray(int [] source){
        ...
        for (int i=0;i<source.length;i++){
            temp[source.length-1-i]=source[i];
        }
        for (int i=0;i<source.length;i++){
            source[i]=temp[i];
        }
    ...
    public static boolean insert(int [] arr, int size, int elem, int index){
        // TO DO
        int nArr[] = copyArray(arr,arr.length);
        if(size>=arr.length){
